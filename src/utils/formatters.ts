...
export const formatCurrency = (amount: number, currency: string | undefined, privacyMode: boolean) => {
  if (privacyMode) return '••••'
  const displayCurrency = currency || useUserStore.getState().currentUser?.defaultCurrency || 'INR'
  return new Intl.NumberFormat('en-IN', { style: 'currency', currency: displayCurrency }).format(amount)
}
