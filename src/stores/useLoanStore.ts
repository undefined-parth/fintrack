  ...
  persist(
    (set, get) => ({
      ...
      addLoan: (payload) => {
        ...
        if (!payload.totalAmount || payload.totalAmount <= 0) return { ok: false, error: 'Total amount must be > 0' }
        ...
        useAuditStore.getState().addEntry({ action: 'create', entity: 'loan', entityId: newLoan.id, newValue: newLoan })
        return { ok: true, data: newLoan }
      },
      updateLoan: (id, updates) => {
        set(state => ({
          loans: state.loans.map(l => l.id === id ? { ...l, ...updates, updatedAt: new Date().toISOString() } : l)
        }))
        useAuditStore.getState().addEntry({ action: 'update', entity: 'loan', entityId: id, newValue: updates })
      },
      deleteLoan: (id) => {
        ...
        useAuditStore.getState().addEntry({ action: 'delete', entity: 'loan', entityId: id, newValue: null })
        
        return { ok: true }
      },
      getLoansForUser: (userId) => get().loans.filter(l => l.userId === userId),
      getActiveLoans: (userId) => get().loans.filter(l => l.userId === userId && l.status === 'active'),
      getLoanSummary: (userId) => {
        const userLoans = get().loans.filter(l => l.userId === userId)
        let totalGiven = 0, totalTaken = 0, totalGivenRemaining = 0, totalTakenRemaining = 0
