    ...
    (set, get) => {
      ...
      const applyTransactionImpact = (tx: Transaction): Result => {
        ...
        if (tx.type === 'income') {
          ...
          if (tx.loanType === 'given') {
            if (account.type === 'credit')
              return { ok: false, error: 'Cannot give a loan from a credit card' };
            if ((account.balance ?? 0) < amount)
              return { ok: false, error: 'Insufficient balance' };
            updateBalance(account.id, -amount);
            ...
            if (account.type === 'credit')
              return { ok: false, error: 'Cannot accept loan settlements into a credit card' };
      ...
      return {
        ...
        addTransaction: (payload): Result<Transaction> => {
          ...
          const applyResult = applyTransactionImpact(tx);
          if (!applyResult.ok) return applyResult as unknown as Result<Transaction>;

          set((state) => ({ transactions: [tx, ...state.transactions] }));

          // Sync tags to user store
          if (tx.tags.length > 0) {
            useUserStore.getState().updateUserTags(tx.userId, tx.tags);
          }
        ...
        updateTransaction: (id, updates) => {
          ...
          // Sync tags to user store
          if (updatedTx.tags.length > 0) {
            useUserStore.getState().updateUserTags(updatedTx.userId, updatedTx.tags);
          }
